...
spherehdl::spherehdl(float radius, int levels, int slices)
{
	...
	for (int i = 0; i < levels; i++)
		...
		{
			rigid[0].indices.push_back(i*slices + j);
			rigid[0].indices.push_back(i*slices + (j+1)%slices);
			rigid[0].indices.push_back((i+1)*slices + j);

			rigid[0].indices.push_back((i+1)*slices + j);
			rigid[0].indices.push_back(i*slices + (j+1)%slices);
			rigid[0].indices.push_back((i+1)*slices + (j+1)%slices);
		}
...
cylinderhdl::cylinderhdl(float radius, float height, int slices)
{
	...
	for (int i = 0; i < slices; i++)
	{
		rigid[0].indices.push_back(1 + (i+1)%(slices+1));
	...
	for (int i = 0; i < slices; i++)
	{
		...
		rigid[0].indices.push_back(1 + (slices+1) + (i+1)%(slices+1));
		rigid[0].indices.push_back(1 + 2*(slices+1) + i);

		rigid[0].indices.push_back(1 + 2*(slices+1) + i);
		rigid[0].indices.push_back(1 + (slices+1) + (i+1)%(slices+1));
		rigid[0].indices.push_back(1 + 2*(slices+1) + (i+1)%(slices+1));
	}

	for (int i = 0; i < slices; i++)
	{
		rigid[0].indices.push_back(1 + 3*(slices+1) + i);
		rigid[0].indices.push_back(1 + 3*(slices+1) + (i+1)%(slices+1));
